...
func calculate_def_effectiveness(reaction_map REACTION_MAP_TYPE, def_skill, atk_skill Skill) int {
  //var max_effectiveness int // from -2 to 2?, -2 = heals from it, 0 = imune, 2 = double dmg 
  reaction_eff := reaction(reaction_map, def_skill.defense.material, atk_skill.defense.material, atk_skill.interaction_type)
...
func use_skill(reaction_map REACTION_MAP_TYPE, s Skill, owner Entity, target Entity) ActionResultABC {
...
func get_bonuses_for_actor(reaction_map REACTION_MAP_TYPE,  s Skill, def_s Skill, ow, tg Entity, interaction_type I9N_ID) int {
  status_bonus := ow.combat.health.get_bonus()
  attr_bonus  := ow.combat.attr.get_bonus(s)
  skill_bonus := s.get_bonus()
  effectiveness_bonus := reaction(reaction_map, s.attack.material, def_s.defense.material, interaction_type).Value
  return int( 
...
func plan_final_bonuses(reaction_map REACTION_MAP_TYPE, s Skill, owner, target Entity) int {
  // 0 = completely misses/does harm to himself, 100 = godlike attack (needs to check lethality and stuff)

  def_s := target.combat.skills.get_defense_skill_vs(reaction_map, s, s.interaction_type)
